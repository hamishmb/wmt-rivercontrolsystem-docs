#!/usr/bin/env python
# -*- coding: utf-8 -*- 
# resistance_probe.py Version 1.2
...
def GetLevel():
    ...
    for Pin in RPins:
        ...
        #Check that all the below pins are also active (probe isn't faulty).
        FaultText = ""

        for Pin in RPins[Index+1:]:
            if Pin != ActiveState:
                FaultText += "Pin "+unicode(Pin)+" is faulty. "

        #Return the level, assume pin 0 is at 0mm. Also return FaultText
        return Index*100, FaultText
...
try:
    while (NumberOfReadingsToTake == 0 or (NumberOfReadingsTaken < NumberOfReadingsToTake)):
        Level, FaultText = GetLevel()

        if FaultText == "":
            print("Time: ", str(datetime.datetime.now()), "Level: "+str(Level), "No faults detected.")
            RecordingsFile.write("Time: "+str(datetime.datetime.now())+" Level: "+str(Level)+" No faults detected\n")

        else:
            print("Time: ", str(datetime.datetime.now()), "Level: "+str(Level), "*** FAULT DETECTED ***: "+FaultText)
            RecordingsFile.write("Time: "+str(datetime.datetime.now())+" Level: "+str(Level)+" *** FAULT DETECTED ***: "+FaultText+"\n")
