...
def GetLevel():
    ...
    for Pin in RPins:
        ...
        for Pin in RPins:
            StateText += "Pin "+unicode(Pin)+" is "+unicode(GPIO.input(Pin))+". "
...
try:
    while (NumberOfReadingsToTake == 0 or (NumberOfReadingsTaken < NumberOfReadingsToTake)):
        Level, StateText = GetLevel()

        print("Time: ", str(datetime.datetime.now()), "Level: "+str(Level), "Pin states: "+StateText)
        RecordingsFile.write("Time: "+str(datetime.datetime.now())+" Level: "+str(Level)+" Pin states: "+StateText+"\n")
